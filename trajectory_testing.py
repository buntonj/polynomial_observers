...
def generate_simulations(system, num_simulations: int, num_steps: int):
    '''
    Generates an array of simulations of system outputs and their derivatives.
    args:
        system (ContinuousTimeystem object): system to simulate.
        num_simulations (int): number of simulation traces to generate
        num_steps (int): number of steps to move forward the simulation
    returns:
        y (np.array) : an array of shape (p, d, num_steps, num_simulations)
    '''
    d = system.ode.nderivs  # how many derivatives can/will we compute?
    y = np.empty((system.p, d, num_steps, num_simulations))
    x = np.empty((system.n, num_steps))
    ...
    for i in range(num_simulations):
        ...
        y[:, :, 0, i] = system.ode.output_derivative(t, x[:, 0], u)

        # evolve simulation forward num_steps-1 more times
        for j in range(1, num_steps):
            u = control_input(t, y[:, 0, j-1, i])
            x[:, j], _ = system.step(u)
            t = system.t
            y[:, :, j, i] = system.ode.output_derivative(t, x[:, j], u)
...
sim_sys = ContinuousTimeSystem(ODE, dt=sampling_dt, solver='RK45')
simulation_data = generate_simulations(sim_sys, num_simulations, num_sim_steps)
traj_estimator = TrajectoryEstimator(simulation_data, sim_sys.dt)
...
theta_traj = np.empty((num_simulations, num_sampling_steps))
yhat_traj = np.empty((d+1, num_sampling_steps))
...
for t in range(1, num_sampling_steps):
    ...
    if t >= N-1:
        ...
        residual_traj[:, t] = traj_estimator.residuals
